...
def label0ToNumClass(y_):
# Original unique classes in sorted order:
    original_classes = np.unique(y_)
    # Create a mapping from old label to new label
    class_to_idx = {cls: i for i, cls in enumerate(original_classes)}
    # Assuming y_train is your labels array
    y_train_remapped = np.array([class_to_idx[label] for label in y_])
    
    return y_train_remapped
...
def get_pamap2_data(verbose=False):
    ...
    train_y = label0ToNumClass(train_y)
    val_y = label0ToNumClass(val_y)
    test_y = label0ToNumClass(test_y)

    # train_y =F.one_hot(torch.tensor(train_y, dtype=torch.int64) - 1, num_classes=19).float()
    # test_y = F.one_hot (torch.tensor(test_y, dtype=torch.int64), num_classes=19).float()
    # val_y = F.one_hot(torch.tensor(val_y, dtype=torch.int64), num_classes=19).float()
    # test_y = tf.keras.utils.to_categorical(test_y, num_classes=19)
    # val_y = tf.keras.utils.to_categorical(val_y, num_classes=19)
    
    if verbose:
        print("unique test_y", np.unique(test_y, return_counts=True))
        print("unique train_y", np.unique(train_y, return_counts=True))
